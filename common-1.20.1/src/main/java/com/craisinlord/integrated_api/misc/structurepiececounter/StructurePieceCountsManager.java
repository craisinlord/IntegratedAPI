...
public class StructurePieceCountsManager extends SimpleJsonResourceReloadListener {
    ...
    private List<StructurePieceCountsObj> getStructurePieceCountsObjs(ResourceLocation fileIdentifier, JsonElement jsonElement) throws Exception {
        List<StructurePieceCountsObj> piecesSpawnCounts = GSON.fromJson(jsonElement.getAsJsonObject().get("integrated_pieces_spawn_counts"), new TypeToken<List<StructurePieceCountsObj>>() {}.getType());
...
//                    IntegratedAPI.LOGGER.error("Integrated API Error: Found {} entry has a condition that does not exist. Extra info: {}", entry.nbtPieceName, fileIdentifier);
                }
            }
        }
    ...
    protected void apply(Map<ResourceLocation, JsonElement> loader, ResourceManager manager, ProfilerFiller profiler) {
        ...
        loader.forEach((fileIdentifier, jsonElement) -> {
            try {
                ...
                IntegratedAPI.LOGGER.error("Integrated API Error: Couldn't parse integrated_pieces_spawn_counts file {} - JSON looks like: {}", fileIdentifier, jsonElement, e);
            }
        });
    ...
    public void parseAndAddCountsJSONObj(ResourceLocation structureRL, List<JsonElement> jsonElements) {
        jsonElements.forEach(jsonElement -> {
            try {
                ...
                IntegratedAPI.LOGGER.error("Integrated API Error: Couldn't parse integrated_pieces_spawn_counts file {} - JSON looks like: {}", structureRL, jsonElement, e);
            }
        });
    }
