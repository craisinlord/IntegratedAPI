...
import static com.craisinlord.integrated_api.IntegratedAPI.GSON;
...
public class StructurePieceCountsManager extends SimpleJsonResourceReloadListener {
    ...
    private List<StructurePieceCountsObj> getStructurePieceCountsObjs(ResourceLocation fileIdentifier, JsonElement jsonElement) throws Exception {
        ...
        for(int i = piecesSpawnCounts.size() - 1; i >= 0; i--) {
            ...
            if(entry.condition != null) {
                Supplier<Boolean> supplier = IAConditionsRegistry.IA_JSON_CONDITIONS_REGISTRY.lookup().get(ResourceLocation.tryParse(entry.condition));
                if (supplier != null) {
                    if(!supplier.get()) {
                    ...
                    IntegratedAPI.LOGGER.error("Repurposed Structures Error: Found {} entry has a condition that does not exist. Extra info: {}", entry.nbtPieceName, fileIdentifier);
                }
            }
        }
    ...
    protected void apply(Map<ResourceLocation, JsonElement> loader, ResourceManager manager, ProfilerFiller profiler) {
        ...
        loader.forEach((fileIdentifier, jsonElement) -> {
            try {
                ...
                IntegratedAPI.LOGGER.error("Repurposed Structures Error: Couldn't parse rs_pieces_spawn_counts file {} - JSON looks like: {}", fileIdentifier, jsonElement, e);
            }
        });
    ...
    public void parseAndAddCountsJSONObj(ResourceLocation structureRL, JsonElement jsonElement) {
        try {
            ...
            IntegratedAPI.LOGGER.error("Repurposed Structures Error: Couldn't parse rs_pieces_spawn_counts file {} - JSON looks like: {}", structureRL, jsonElement, e);
        }
    }
