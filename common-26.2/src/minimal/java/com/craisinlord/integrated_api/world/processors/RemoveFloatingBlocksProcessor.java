...
public class RemoveFloatingBlocksProcessor implements StructureProcessor {
    ...
    @Override
    public StructureTemplate.StructureBlockInfo processBlock(LevelReader levelReader, BlockPos pos, BlockPos piecePos, BlockPos blockPos, StructureTemplate.StructureBlockInfo structureBlockInfoWorld, StructurePlaceSettings structurePlacementData) {
        ...
        if (structureBlockInfoWorld.state().isAir() || !structureBlockInfoWorld.state().getFluidState().isEmpty()) {
            cachedChunk.setBlockState(mutable, structureBlockInfoWorld.state());
            ...
            for (Direction direction : Direction.Plane.HORIZONTAL) {
                ...
                if (!sideBlock.canSurvive(levelReader, mutable)) {
                    chunkAccess2.setBlockState(mutable, structureBlockInfoWorld.state());
                }
            }
        }
