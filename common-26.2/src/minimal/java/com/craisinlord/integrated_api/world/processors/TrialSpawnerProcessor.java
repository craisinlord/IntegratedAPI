...
public final class TrialSpawnerProcessor implements StructureProcessor {
    public static final MapCodec<TrialSpawnerProcessor> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
            Identifier.CODEC.fieldOf("loot_table").forGetter(c -> c.lootTable), Identifier.CODEC.optionalFieldOf("ominous_loot_table").forGetter(c -> c.ominousLootTable),
            Identifier.CODEC.fieldOf("integrated_api_spawner_resourcelocation").forGetter(c -> c.spawnerIdentifier), InclusiveRange.INT.optionalFieldOf("valid_block_light_level").forGetter(c -> c.validBlockLightLevel),
    ...
    private final Identifier lootTable, spawnerIdentifier;
    ...
    private TrialSpawnerProcessor(Identifier lootTable, Optional<Identifier> ominousLootTable, Identifier spawnerIdentifier, Optional<InclusiveRange<Integer>> validBlockLightLevel, Optional<InclusiveRange<Integer>> validSkyLightLevel, boolean replaceVanillaSpawners, Optional<CompoundTag> normalConfig, Optional<CompoundTag> ominousConfig, Optional<Integer> targetCooldownLength, Optional<Integer> requiredPlayerRange) {
        this.lootTable = lootTable; this.ominousLootTable = ominousLootTable; this.spawnerIdentifier = spawnerIdentifier; this.validBlockLightLevel = validBlockLightLevel; this.validSkyLightLevel = validSkyLightLevel; this.replaceVanillaSpawners = replaceVanillaSpawners; this.normalConfig = normalConfig; this.ominousConfig = ominousConfig; this.targetCooldownLength = targetCooldownLength; this.requiredPlayerRange = requiredPlayerRange;
    }

    @Override
    public StructureTemplate.StructureBlockInfo processBlock(LevelReader level, BlockPos pos, BlockPos piecePos, BlockPos blockPos, StructureTemplate.StructureBlockInfo worldInfo, StructurePlaceSettings settings) {
    ...
    @Override
    public MapCodec<? extends StructureProcessor> codec() { return CODEC; }
}
