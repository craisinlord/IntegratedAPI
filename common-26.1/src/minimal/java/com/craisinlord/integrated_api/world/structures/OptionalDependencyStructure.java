...
public class OptionalDependencyStructure extends JigsawStructure {
    ...
    public OptionalDependencyStructure(StructureSettings config, Holder<StructureTemplatePool> startPool, int size, Optional<YRangeAllowance> yAllowance, HeightProvider startHeight, Optional<Heightmap.Types> projectStartToHeightmap, boolean cannotSpawnInLiquid, Optional<Integer> terrainHeightCheckRadius, Optional<Integer> allowedTerrainHeightRange, Optional<Integer> biomeRadius, Optional<Integer> maxDistanceFromCenter, Optional<String> requiredMod, Optional<String> illegalMod, boolean rotationFixed, EnhancedTerrainAdaptation enhancedTerrainAdaptation, LiquidSettings liquidSettings) {
        super(config, startPool, size, yAllowance, startHeight, projectStartToHeightmap, cannotSpawnInLiquid, terrainHeightCheckRadius, allowedTerrainHeightRange, biomeRadius, maxDistanceFromCenter, Optional.empty(), rotationFixed, enhancedTerrainAdaptation, liquidSettings);
    ...
    protected boolean extraSpawningChecks(GenerationContext context, BlockPos blockPos) {
        if (!super.extraSpawningChecks(context, blockPos)) {
            return false;
        }
        if (this.requiredMod.isPresent()) {
            ArrayList<String> requiredMods = convertModList(this.requiredMod.get());
            for (String mod : requiredMods) {
                if (!PlatformHooks.isModLoaded(mod)) {
                    IntegratedAPI.LOGGER.debug("Attempted to spawn Integrated API structure but not all required mods {} are present.", requiredMods);
                    return false;
                }
            }
        }
        if (this.illegalMod.isPresent()) {
            ArrayList<String> illegalMods = convertModList(this.illegalMod.get());
            for (String mod : illegalMods) {
                if (PlatformHooks.isModLoaded(mod)) {
                    IntegratedAPI.LOGGER.debug("Attempted to spawn Integrated API structure but illegal mods {} are present.", illegalMods);
                    return false;
                }
            }
        }
