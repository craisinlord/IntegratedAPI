...
public class MobSpawnerManager extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<Identifier, JsonElement> loader, ResourceManager manager, ProfilerFiller profiler) {
        ...
        loader.forEach((fileIdentifier, jsonElement) -> {
            try {
                ...
                IntegratedAPI.LOGGER.error("Integrated API Error: Couldn't parse spawner mob list {}", fileIdentifier, e);
            }
        });
    ...
    public EntityType<?> getSpawnerMob(Identifier spawnerJsonEntry, RandomSource random) {
        ...
        if(spawnerMobEntries == null) {
            IntegratedAPI.LOGGER.log(Level.ERROR,"\n***************************************\nFailed to get mob. Please check that "+spawnerJsonEntry+".json is correct or that no other mod is interfering with how vanilla reads data folders. Let CraisinLord know about this too!\n***************************************");
        ...
        try {
            while(true) {
                ...
                if(randomWeight <= 0) {
                    return BuiltInRegistries.ENTITY_TYPE.get(Identifier.tryParse(spawnerMobEntries.get(index).name));
                }
            ...
            IntegratedAPI.LOGGER.log(Level.ERROR,"\n***************************************\nFailed to get mob. Please check that "+spawnerJsonEntry+".json is correct and let CraisinLord know he broke the mob spawner code!\n***************************************");
